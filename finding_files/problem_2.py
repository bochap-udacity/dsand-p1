...
def find_files(suffix, path):
    ...
    def recurse_path(search_path):
        if isfile(search_path) == True:
            if search_path.endswith(suffix) == True:
                found_paths.append(relpath(search_path, BASE_DIR))
        else:
            for child_path in listdir(search_path):
                recurse_path(join(search_path, child_path))
